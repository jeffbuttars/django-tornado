...
if tornado_version_major == 3:
    ...
    from django_tornado.core.tv32.application import DjangoTornadoRequestHandler, DjangoApplication
elif tornado_version_major == 4:
    if tornado_version_minor != 0:
        raise TornadoVersionNotSupported(
            "Tornado version %s is not supported by DjangoTornado." % tornado.version)
    from django_tornado.core.tv40.application import DjangoTornadoRequestHandler, DjangoApplication
else:
    raise TornadoVersionNotSupported(
        "Tornado version %s is not supported by DjangoTornado." % tornado.version)
